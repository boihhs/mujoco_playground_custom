...
from mujoco_playground._src.locomotion.zeroth import base as zeroth_base
from mujoco_playground._src.locomotion.zeroth import zeroth_constants as consts
...
class Joystick(zeroth_base.ZerothEnv):
  ...
  def _post_init(self) -> None:
    ...
    self._kp = jp.array([
        20.0, 20.0, 20.0,  # Left arm.
        20.0, 20.0, 20.0,  # Right arm.
        25.0, 25.0, 25.0, 25.0, 25.0,  # Left leg.
        25.0, 25.0, 25.0, 25.0, 25.0 # Right leg.
    ])

    self._kd = jp.array([
        3.0, 3.0, 3.0,  # Left arm.
        3.0, 3.0, 3.0,  # Right arm.
        5.0, 5.0, 5.0, 5.0, 1.0,  # Left leg.
        0.01, 1.0, 1.0, 0.01, 1.0  # Right leg.
    ])
    # fmt: on
    ...
    self._left_foot_box_geom_id = self._mj_model.geom("foot_left_collision").id
    self._right_foot_box_geom_id = self._mj_model.geom("foot_right_collision").id

    # Contact sensor IDs.
    self._left_foot_floor_found_sensor = [
        self._mj_model.sensor(f"left_foot_floor_found").id
    ]
    self._right_foot_floor_found_sensor = [
        self._mj_model.sensor(f"right_foot_floor_found").id
    ]
  ...
  def _cost_feet_slip(
    ...
    vel_xy = feet_vel[..., :2]
    reward = jp.sum(jp.linalg.norm(vel_xy, axis=-1) * contact)
    return reward
